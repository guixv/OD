...
from timm.models._efficientnet_builder import BlockArgs, EfficientNetBuilder, decode_arch_def, efficientnet_init_weights, \
...
if __name__ == '__main__':
    ...
    model = mobilenetv4_conv_blur_medium(num_classes=1000, features_only=True,pretrained= True)
    
    # model = mobilenetv4_conv_medium(num_classes=1000, features_only=False,pretrained= False)
    model = model.eval()
    import os
    out = model(x)
    ...
    model.state_dict()
    last_path = os.path.join("MNV4.pth")
    torch.save(model.state_dict(), last_path)
    print("logit: ", out[0].size())
    # print("logit: ", out[1].size())
    # print("logit: ", out[2].size())
    # print("logit: ", out[3].size())
    # print("logit: ", out[4].size())
    ...
    torch.onnx.export(model, x, 'MNV4_mid_blur.onnx', opset_version=12)
